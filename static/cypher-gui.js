function showCypherResult(result, elem) {
  ...
  if (!Array.isArray(result)) {
    var msg = result instanceof Error ? result.message.replaceAll("<","&lt;") : 
      "<p>Unable to get response from endpoint. Network error or endpoint is down.</p>"
    $(`<div class="errorResult"><div class="redOutline">${msg}</div></div>`).appendTo(elem)
    return
  } 
  ...
  async query() {
    ...
    return fetch(this.endpoint, { method: "POST", body: cypher })
      .then(async res => {
        ...
        if (res.ok) {return content} else {
          throw new Error(`${JSON.stringify(content)}`)
        }
      })
      .then(res => { 
        this.emit("queryResponse", res, Date.now() - queryStart) 
      })
      .catch(e => {
        // Response is no JSON or network error
